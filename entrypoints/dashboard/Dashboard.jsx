...
function Dashboard() {
    ...
    const { loading, workspaces, lastSession, getWorkspaces } = useContext(BucketContext);
                    ...
                    <div className='w-full h-full pb-6'>
                        {
                            (tag === defaultWorkspaces.LAST_SESSION) ? <>
                                ...
                                <div className="w-full h-auto flex flex-col gap-1">
                                    {
                                        lastSession?.map(({ id, title, favIconUrl, url }, index, array) => {
                                            return <a className='w-fit hover:underline hover:text-blue-200 cursor-pointer flex items-center gap-2 text-white' target='_blank' key={id} href={url}>
                                ...
                                :
                                <BucketsContainer buckets={newListOfBuckets} loading={loading} tag={tag} />
                        }
                    </div>
                </main>
            </div >
